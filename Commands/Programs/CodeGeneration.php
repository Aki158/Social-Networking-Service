...
class CodeGeneration extends AbstractCommand
{
    ...
    public function execute(): int
    {
        ...
        if ($codeGenType === 'seederDao') {
            $seederDaoName = $this->getArgumentValue('name');
            $this->generateSeederDaoFile($seederDaoName);
        }
...
MIGRATION;
    ...
    private function generateSeederDaoFile(string $seederDaoName): void
    {
        $filename = sprintf(
            '%s.php',
            $seederDaoName
        ...
        $seederDaoContent = $this->getSeederDaoContent($seederDaoName);

        // 移行ファイルを保存するパスを指定します
        $path = sprintf("%s/../../Database/SeedsDao/%s", __DIR__,$filename);

        file_put_contents($path, $seederDaoContent);
        $this->log("Seeder DAO file {$filename} has been generated!");
    }

    private function getSeederDaoContent(string $seederDaoName): string
    {
        $className = $this->pascalCase($seederDaoName);
        if(!strstr($className, 'DaoSeeder')){
            $className .= 'DaoSeeder';
        }
...
class {$className} implements SeederDao{

    public function seed(): bool
    {
        // TODO: seed()メソッドを実装してください。
        return true;
    }
}
SeedDao;
